...
template <unsigned End, typename Char, typename... Chars> struct CTStringSliceUpImpl;
...
struct CTStringSliceUp {
  using type = typename CTStringSliceUpImpl<End, Chars...>::type;
};

template <typename... Chars>
struct CTStringSliceUp<0, Chars...> {
  using reversed_type = typename reverse<pack<Chars...>>::type;
  using type = typename CTStringSliceResult<reversed_type>::type;
};

template <unsigned End, typename Char, typename... Chars>
struct CTStringSliceUpImpl {
  using type = typename CTStringSliceUp<End - 1, Chars...>::type;
};

template <unsigned End, typename... Chars>
struct CTStringSliceUpUnpack<End, pack<Chars...>> {
  using type = typename CTStringSliceUp<End, Chars...>::type;
};

template <unsigned End, typename... Chars>
struct CTStringSliceReverse {
  using reversed_type = typename reverse<pack<Chars...>>::type;
  using type = typename CTStringSliceUpUnpack<sizeof...(Chars) - End, reversed_type>::type;
};

template <unsigned Begin, unsigned End, typename Char, typename... Chars>
struct CTStringSliceDown {
  using type = typename CTStringSliceDown<Begin - 1, End - 1, Chars...>::type;
};

template <unsigned End, typename Char, typename... Chars>
struct CTStringSliceDown<0, End, Char, Chars...> {
  using type = typename CTStringSliceReverse<End, Char, Chars...>::type;
};

template <unsigned Begin, unsigned End, typename... Chars>
struct CTSlice<Begin, End, CTString<Chars...>> {
  using type = typename CTStringSliceDown<Begin, End, Chars...>::type;
...
private:
  ...
  static constexpr unsigned start_after_nth_comma() noexcept {
    if constexpr (n == 0 || index >= String::size() - 1) {
      ...
      return start_after_nth_comma<n, count + (String::value[index] == ',' && open_groups == 0), index + 1, open_groups + open_groups_delta>();
    }
  }

  template <unsigned n, unsigned count = 0, unsigned index = 0, unsigned open_groups = 0>
  static constexpr unsigned end_after_nth_comma() noexcept {
    if constexpr ((index >= String::size() - 1) || (String::value[index] == ',' && count + 1 == n + 1 && open_groups == 0)) {
      return index;
    } else {
      constexpr int open_groups_delta = (String::value[index] == '(') - (String::value[index] == ')') + (String::value[index] == '[') - (String::value[index] == ']') + (String::value[index] == '{') - (String::value[index] == '}');
      return end_after_nth_comma<n, count + (String::value[index] == ',' && open_groups == 0), index + 1, open_groups + open_groups_delta>();
    }
  }

  template <unsigned n>
  using nth_segment = typename CTSlice<start_after_nth_comma<n>(), end_after_nth_comma<n>(), String>::type;
};
